...
class SSLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        origin_qry_image =sitk.ReadImage(self.sup_file[idx]) 
        origin_qry_img_info = self.sup_file[idx]
        origin_qry_img_name = Path(self.sup_file[idx]).name
        ...
        sample = {"sup_img":sup_img, "sup_fg_label":sup_label_fg, "qry_img":qry_img, "qry_label_fg":qry_label_fg,
         "sup_bg_label":sup_label_bg, "qry_label_bg":qry_label_bg, "origin_qry_img_info":origin_qry_img_info, "origin_qry_img_name":origin_qry_img_name}
...
class SSL3DUNet(torch.nn.Module):
    ...
    def forward(self,batch):
        ...
        sup_img = batch["sup_img"].to('cuda:0')
        qry_img = batch["qry_img"].to('cuda:0')
...
def _pass_compose(sup_fts, qry_fts, sup_fg_label, sup_bg_label, qry_label_fg, qry_label_bg,params, thresh=.95):
    ...
    raw_score_bg = _alp_module(qry_fts,sup_fts,qry_label_bg,params,mode='local',thresh=thresh)
    # NO_OVER_THRES =  torch.all(torch.flatten(sup_fg_label < thresh))
    NO_OVER_THRES = False 
    raw_score_fg = _alp_module(qry_fts,sup_fts,qry_label_fg,params,mode='global' if not NO_OVER_THRES else 'mask',thresh=thresh)
...
def MyLoss(epoch, sup_fg_label,sup_bg_label,qry_label_fg,qry_label_bg, qry_fts, sup_fts,params, writer:SummaryWriter):
    criterion = Diceloss()

    qry_pred, sup_pred = _pass_compose(sup_fts, qry_fts, sup_fg_label, sup_bg_label, qry_label_fg, qry_label_bg,params)
#############################################
 # saving result 
    vis_arr = torch.clone(qry_pred) 
    vis_arr = nn.functional.interpolate(vis_arr,size=(24,256,256))
    vis_arr = vis_arr.squeeze(0)
    vis_arr = vis_arr.squeeze(0)
    data_arr = vis_arr.cpu().detach().numpy()
    numpy.save("predict_result/result_"+str(epoch)+".pixel",data_arr)

    vis_arr_fg = torch.clone(qry_label_fg) 
    vis_arr_fg = nn.functional.interpolate(vis_arr_fg,size=(24,256,256))
    vis_arr_fg = vis_arr_fg.squeeze(0)
    vis_arr_fg = vis_arr_fg.squeeze(0)
    data_arr = vis_arr_fg.cpu().detach().numpy()
    numpy.save("query_result/result_"+str(epoch)+".pixel",data_arr)

    query_loss = criterion((qry_pred),(qry_label_fg))
    # sup_loss = criterion((sup_pred),(sup_fg_label))
    # loss = torch.add(query_loss , sup_loss )
    return None,None, query_loss
###############################################
...
if __name__ == '__main__':
    ...
    model_load_path = "save_model/best_model_184.pth"
    ...
    for epoch in range(epochs):
        ...
        # train_loader = DataLoader(dataset,batch_size=1,num_workers=12,shuffle=True,persistent_workers=True)
        # val_loader = DataLoader(dataset,batch_size=1,num_workers=12,persistent_workers=True)
        test_loader = DataLoader(dataset,batch_size=1,num_workers=12,persistent_workers=True)
        ...
        with torch.no_grad():
            for test_batch in test_loader:
                test_sup_fg_label = test_batch["sup_fg_label"].to(device)
                test_sup_bg_label = test_batch["sup_bg_label"].to(device)
                test_qry_label_fg = test_batch["qry_label_fg"].to(device)
                test_qry_label_bg = test_batch["qry_label_bg"].to(device)
                test_sup_img = test_batch["sup_img"].to(device)
                test_qry_img = test_batch["qry_img"].to(device)

                test_prediction = model(test_batch) 

                _,_,test_query_loss = MyLoss(epoch,test_sup_fg_label,test_sup_bg_label,test_qry_label_fg,test_qry_label_bg, test_prediction["qry_fts"],test_prediction["sup_fts"],model_param,writer) 
                # writer.add_scalar("Accuracy/test_query_loss", test_query_loss,epoch)
                # writer.add_scalar("Accuracy/test_sup_loss", test_sup_loss,epoch)
                # writer.add_scalar('loss/valid', test_loss,epoch)
                print(f"test_query_loss:{test_query_loss}")
                test_loss_sum += test_query_loss.item()
                test_count +=1
        print(f"Final mean loss:{test_loss_sum/test_count}")
    ...
    #         # just training
    #         train_loss,sup_loss,query_loss = MyLoss(epoch,sup_fg_label,sup_bg_label,
    ...
    #                 val_loss,val_sup_loss,val_query_loss = MyLoss(epoch,val_sup_fg_label,val_sup_bg_label,val_qry_label_fg,val_qry_label_bg, val_prediction["qry_fts"],val_prediction["sup_fts"],model_param,writer) 
