...
def _pass_compose(sup_fts, qry_fts, sup_fg_label, sup_bg_label, qry_label_fg, qry_label_bg,params, thresh=.95):
    ...
    raw_score_bg = _alp_module(qry_fts,sup_fts,qry_label_fg,params,mode='local',thresh=thresh)
    # NO_OVER_THRES =  torch.all(torch.flatten(sup_fg_label < thresh))
    NO_OVER_THRES = False 
    raw_score_fg = _alp_module(qry_fts,sup_fts,qry_label_bg,params,mode='global' if not NO_OVER_THRES else 'mask',thresh=thresh)
...
def MyLoss(epoch, sup_fg_label,sup_bg_label,qry_label_fg,qry_label_bg, qry_fts, sup_fts,params, writer:SummaryWriter):
    ...
    # saving result 
    data_arr = qry_pred.cpu().detach().numpy()
    save_f = data_arr[0]
    save_f = numpy.squeeze(save_f)

    save_f[numpy.where(save_f>0.99)] = 0

    xf,yf,zf =save_f.nonzero()

    fig_f = plt.figure()
    ax_f = fig_f.add_subplot(111, projection='3d')
    ax_f.scatter(xf,yf,zf,alpha=0.4)
    writer.add_figure('prediction_f',fig_f,epoch)
...
if __name__ == '__main__':
    ...
    sup_img_file = "sup_img_file_copy.txt"
    sup_label_file = "sup_label_file_copy.txt"
    qry_img_file = "qry_img_file_copy.txt"
    qry_label_file = "qry_label_file_copy.txt"
    ...
    model_load_path = "save_model/best_model_97.pth"
    ...
    for epoch in range(epochs):
        ...
        train_loader = DataLoader(train,batch_size=1,num_workers=12,shuffle=True,persistent_workers=True)
        val_loader = DataLoader(val,batch_size=1,num_workers=12,persistent_workers=True)
